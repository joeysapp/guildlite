#pragma once
// Portable copy of GuildWarsMapBrowser FFNA_ModelFile.h — PARSE PATH ONLY.
// GetMesh() (the sole DirectX/AMAT/Mesh-coupled method) has been removed; the
// parsed geometry lives in geometry_chunk.models and is consumed by model.cpp.
...
typedef uint32_t DWORD; // shim: the parser uses DWORD only as a 32-bit integer
                  ...
                  bool& parsed_correctly)
    {
        ...
        if (sub_1.num_models > 0)
        {
            ...
            if (prev_parsed_correctly != parsed_correctly)
            {
                ...
                parsed_correctly = true; // datcore fix: was '==' (no-op) upstream; the
                                         // typo blocked geometry parse when textures failed
...
struct TextureFileNamesChunk
{
                          ...
                          bool& textures_parsed_correctly)
    {
        ...
        actual_num_texture_filenames = std::min<size_t>(num_texture_filenames, (chunk_size - 4) / sizeof(TextureFileName)); // datcore: explicit <size_t> (clang strictness)
