// Portable copy of GuildWarsMapBrowser AtexDecompress.cpp — the ATEX pre-DXT
// unpack DRIVER + the C-ported SubCode1/2/3. SubCode3/4/5/7 (x86 asm upstream)
// are stubbed in atex_stub.cpp; a texture needing them sets the needed-asm flag.
#include "datcore/atex_decompress.h"
...
void AtexDecompress(unsigned int* InputBuffer, unsigned int BufferSize, unsigned int ImageFormat, const SImageDescriptor& ImageDescriptor, unsigned int* OutBuffer)
{
    ...
    if (CompressionCode)
    {
        ...
        {
#ifdef DATCORE_HAVE_ATEX_ASM
            AtexSubCode3_Asm(OutBuffer, DcmpBuffer1, DcmpBuffer2, &ImageData, BlockCount, BlockSize);
#else
            // datcore (non-MSVC): portable C port instead of the x86 asm. Upstream
            // prefers the asm because this "loses some highlights", but it covers
            // the most common non-asm texture path natively.
        ...
        {
            AtexSubCode5_Asm((uintptr_t)OutBuffer + AlphaDataSize2 + AlphaDataSize * 4,
                          (uintptr_t)DcmpBuffer1, (uintptr_t)DcmpBuffer2, (uintptr_t)&ImageData,
    ...
    {
        AtexSubCode7_Asm((uintptr_t)OutBuffer, BlockCount);
    }
